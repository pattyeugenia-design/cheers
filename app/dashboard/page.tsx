...
export default function Dashboard() {
  ...
  const nombre = user?.user_metadata?.name?.split(' ')[0] || 'Patty'

  function slugToPath(slug: string) {
    // Si el slug ya tiene /, usarlo directo. Si no, construir con usuario
          ...
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              {celebraciones.map(cel => (
                ...
                >
                  <div style={{ fontSize: 28, flexShrink: 0 }}>{EMOJIS[cel.tipo] || '✨'}</div>
