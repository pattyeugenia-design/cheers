...
export default function Home() {
  ...
  const F = ({ size = 14, color = '#1D9E75' }) => <span style={{ color, fontSize: size }}>✓</span>
  ...
  return (
    <main style={{ minHeight: '100vh', background: '#fff', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif', color: '#1d1d1f' }}>
      ...
      {/* HERO */}
      <section style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 60, padding: '80px 60px', maxWidth: 1100, margin: '0 auto', alignItems: 'center' }}>
        <div>
        ...
        <div style={{ background: '#f5f5f7', borderRadius: 24, padding: 24, border: '0.5px solid #e0e0e0' }}>
          <div style={{ background: 'linear-gradient(135deg, #534AB7 0%, #D4537E 100%)', borderRadius: 16, padding: '20px 20px 16px', marginBottom: 12, textAlign: 'center', color: '#fff' }}>
            <p style={{ fontSize: 32, margin: '0 0 6px' }}>🎂</p>
            <p style={{ fontSize: 18, fontWeight: 500, margin: '0 0 4px' }}>Los 30 de Rodrigo</p>
          ...
          <div style={{ background: '#fff', borderRadius: 12, padding: 14, marginBottom: 10 }}>
            <p style={{ fontSize: 11, color: '#aeaeb2', fontWeight: 500, marginBottom: 10, letterSpacing: '0.5px' }}>🗺️ {tx.demo_plan.toUpperCase()}</p>
            {[
              ...
              <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: i < 2 ? 10 : 0, paddingBottom: i < 2 ? 10 : 0, borderBottom: i < 2 ? '0.5px solid #f0f0f0' : 'none' }}>
                <span style={{ fontSize: 11, color: '#D4537E', fontWeight: 500, minWidth: 28, paddingTop: 1 }}>{p.hora}</span>
                <div>
          ...
          <div style={{ background: '#fff', borderRadius: 12, padding: 14 }}>
            <p style={{ fontSize: 11, color: '#aeaeb2', fontWeight: 500, marginBottom: 10, letterSpacing: '0.5px' }}>🎁 {tx.demo_gifts.toUpperCase()}</p>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              {['🛍️ Liverpool', '📦 Amazon', '🌸 EnviaFlores'].map(g => (
                <span key={g} style={{ fontSize: 12, padding: '5px 12px', background: '#f5f5f7', borderRadius: 8, color: '#6e6e73', border: '0.5px solid #e0e0e0' }}>{g}</span>
              ))}
            </div>
          </div>
        </div>
      ...
      <section style={{ padding: '70px 40px', background: '#fff' }}>
        <div style={{ maxWidth: 700, margin: '0 auto', textAlign: 'center' }}>
          ...
          <div style={{ background: '#f5f5f7', borderRadius: 20, padding: '40px 40px 32px', minHeight: 160, position: 'relative' }}>
            <p style={{ fontSize: 22, color: '#D4537E', marginBottom: 16 }}>"</p>
            <p style={{ fontSize: 18, color: '#1d1d1f', lineHeight: 1.6, marginBottom: 20, fontWeight: 400 }}>
