...
const translations = {
  es: {
    nav_how: 'Cómo funciona', nav_prices: 'Precios', nav_cta: 'Entrar',
    h1: 'La celebración,\nen un link.',
    ...
    demo_gift_items: [{ name: 'Juego de copas', taken: true }, { name: 'Fondo para el viaje', taken: false }],
    demo_rsvp: 'Confirmar asistencia',
    demo_confirmed: '12 confirmados',
    how_label: 'CÓMO FUNCIONA',
    how_title: 'Tres pasos.\nTodo resuelto.',
    steps: [
      { n: '01', title: 'Crea', desc: 'Elige el tipo de celebración y arma el plan en minutos.' },
    ...
    plans: [
      { name: 'Free', price: '$0', sub: 'para siempre', features: ['1 celebración activa', 'Hasta 3 invitados', 'Itinerario básico'], cta: 'Empezar gratis', featured: false },
      { name: 'Pro', price: '$9', sub: 'por celebración', features: ['Hasta 10 invitados', 'Itinerario completo', 'Lista de regalos', 'Modo sorpresa', 'Personalización'], cta: 'Empezar con Pro', featured: true, badge: 'MÁS POPULAR' },
      { name: 'Lifetime', price: '$49', sub: 'pago único · para siempre', features: ['Invitados ilimitados', 'Link público para eventos abiertos', 'Historial completo', 'Todo incluido'], cta: 'Comprar Lifetime', featured: false },
    ],
  ...
  en: {
    ...
    demo_gift_items: [{ name: 'Wine glasses set', taken: true }, { name: 'Travel fund', taken: false }],
    demo_rsvp: 'Confirm attendance',
    demo_confirmed: '12 confirmed',
    how_label: 'HOW IT WORKS',
    how_title: 'Three steps.\nEverything sorted.',
    steps: [
      { n: '01', title: 'Create', desc: 'Choose the celebration type and build the plan in minutes.' },
    ...
    plans: [
      { name: 'Free', price: '$0', sub: 'forever', features: ['1 active celebration', 'Up to 3 guests', 'Basic itinerary'], cta: 'Start free', featured: false },
      { name: 'Pro', price: '$9', sub: 'per celebration', features: ['Up to 10 guests', 'Full itinerary', 'Gift list', 'Surprise mode', 'Personalization'], cta: 'Start with Pro', featured: true, badge: 'MOST POPULAR' },
      { name: 'Lifetime', price: '$49', sub: 'one-time · forever', features: ['Unlimited guests', 'Public link for open events', 'Full history', 'Everything included'], cta: 'Buy Lifetime', featured: false },
    ],
    cta_title: 'Ready to celebrate?',
    cta_sub: 'Create your first celebration in less than 2 minutes.',
    cta_btn: 'Start free',
    no_cc2: 'Free forever · No card needed',
...
export default function Home() {
  ...
  useEffect(() => {
    const l = navigator.language?.toLowerCase().startsWith('en') ? 'en' : 'es'
    setLang(l as 'es'|'en')
  ...
  useEffect(() => {
    ...
    const particles: any[] = []
    for (let i = 0; i < 50; i++) particles.push({ x: Math.random()*window.innerWidth, y: Math.random()*window.innerHeight, size: Math.random()*2+.5, vx: (Math.random()-.5)*.25, vy: (Math.random()-.5)*.25, opacity: Math.random()*.4+.1, color: Math.random()>.5?'#534AB7':'#D4537E' })
    const draw = () => {
      ctx.clearRect(0,0,window.innerWidth,window.innerHeight)
      particles.forEach(p => { p.x+=p.vx; p.y+=p.vy; if(p.x<0)p.x=window.innerWidth; if(p.x>window.innerWidth)p.x=0; if(p.y<0)p.y=window.innerHeight; if(p.y>window.innerHeight)p.y=0; ctx.beginPath(); ctx.arc(p.x,p.y,p.size,0,Math.PI*2); ctx.fillStyle=p.color; ctx.globalAlpha=p.opacity; ctx.fill() }); ctx.globalAlpha=1
  ...
  if (cargando) return (
    <main style={{ minHeight:'100vh', background:'linear-gradient(160deg,#0d0b1a,#1a1440)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:F }}>
      <div style={{ textAlign:'center' }}>
        <div style={{ fontSize:32, fontWeight:900, background:'linear-gradient(135deg,#a89df0,#f08cb0)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', letterSpacing:'-1px', marginBottom:16 }}>Cheers</div>
        <style>{`@keyframes lb{0%,100%{opacity:.3;transform:scaleX(.4)}50%{opacity:1;transform:scaleX(1)}}`}</style>
        <div style={{ width:32, height:3, borderRadius:99, background:'linear-gradient(90deg,#534AB7,#D4537E)', margin:'0 auto', animation:'lb 1.2s ease-in-out infinite' }} />
      </div>
    </main>
  )
  ...
  return (
    <main style={{ minHeight:'100vh', fontFamily:F, color:'#fff', background:'#0d0b1a', overflowX:'hidden' }}>
      <style>{`
        ...
        @keyframes bubble{0%{transform:translateY(0);opacity:.8}100%{transform:translateY(-55px);opacity:0}}
        .btn-p:hover{transform:translateY(-2px);box-shadow:0 20px 48px rgba(212,83,126,.55)!important}
        .btn-p{transition:transform .15s,box-shadow .15s}
        .card-h{transition:transform .2s}.card-h:hover{transform:translateY(-4px)}
        .nav-a{transition:color .15s}.nav-a:hover{color:#fff!important}
      `}</style>
      ...
      <nav style={{ position:'fixed', top:0, left:0, right:0, zIndex:100, padding:`0 ${px}`, height:isMobile?56:64, display:'flex', alignItems:'center', justifyContent:'space-between', background:scrolled?'rgba(13,11,26,.9)':'transparent', backdropFilter:scrolled?'blur(20px)':'none', borderBottom:scrolled?'1px solid rgba(255,255,255,.06)':'none', transition:'all .3s' }}>
        <div style={{ fontSize:20, fontWeight:900, background:'linear-gradient(135deg,#a89df0,#f08cb0)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', letterSpacing:'-.5px' }}>Cheers</div>
      ...
      {/* HERO */}
      <section style={{ minHeight:'100vh', display:'flex', alignItems:'center', padding:isMobile?'100px 20px 60px':`80px ${px} 60px`, position:'relative', zIndex:1 }}>
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', width:'100%', display:'grid', gridTemplateColumns:isMobile?'1fr':'1fr 1fr', gap:isMobile?48:80, alignItems:'center' }}>
          ...
          <div>
            ...
            <h1 style={{ fontSize:isMobile?40:72, fontWeight:900, letterSpacing:isMobile?'-2px':'-3px', lineHeight:1, margin:'0 0 20px', color:'#fff' }}>
              {tx.h1.split('\n').map((line,i) => <span key={i}>{i===0?line:<><br/><span style={{ background:'linear-gradient(135deg,#a89df0,#f08cb0)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>{line}</span></>}</span>)}
            </h1>
            <p style={{ fontSize:isMobile?15:18, color:'rgba(255,255,255,.5)', lineHeight:1.7, margin:'0 0 36px' }}>
              {tx.sub.split('\n').map((line,i) => <span key={i}>{line}{i===0&&<br/>}</span>)}
            </p>
            <button className="btn-p" onClick={loginConGoogle} style={{ display:'inline-flex', alignItems:'center', gap:10, padding:isMobile?'14px 22px':'16px 28px', background:'linear-gradient(135deg,#534AB7,#D4537E)', border:'none', borderRadius:16, color:'#fff', fontSize:isMobile?15:16, fontWeight:700, cursor:'pointer', fontFamily:F, boxShadow:'0 12px 32px rgba(212,83,126,.4)', marginBottom:14, width:isMobile?'100%':'auto', justifyContent:'center' }}>
          ...
          <div style={{ display:'flex', justifyContent:'center' }}>
            <div style={{ animation:'float 5s ease-in-out infinite', width:'100%', maxWidth:340, position:'relative' }}>
              <div style={{ background:'rgba(255,255,255,.04)', border:'1px solid rgba(255,255,255,.08)', borderRadius:28, overflow:'hidden', backdropFilter:'blur(20px)', boxShadow:'0 40px 100px rgba(0,0,0,.6)' }}>
                <div style={{ background:'linear-gradient(135deg,#534AB7,#D4537E)', padding:'20px', textAlign:'center' }}>
                  <div style={{ fontSize:11, fontWeight:800, color:'rgba(255,255,255,.7)', letterSpacing:'1.5px', marginBottom:5 }}>BDAY</div>
                  <div style={{ fontSize:18, fontWeight:850, color:'#fff', marginBottom:3 }}>{tx.demo_event}</div>
                ...
                <div style={{ padding:'14px 16px' }}>
                  ...
                  {tx.demo_stops.map((p,i) => <div key={i} style={{ display:'flex', gap:8, marginBottom:i<2?8:12, paddingBottom:i<2?8:0, borderBottom:i<2?'1px solid rgba(255,255,255,.05)':'none' }}>
                    <span style={{ fontSize:10, fontWeight:700, color:'#a89df0', minWidth:26, flexShrink:0 }}>{p.hora}</span>
                    <div><div style={{ fontSize:12, fontWeight:600, color:'#fff' }}>{p.lugar}</div><div style={{ fontSize:10, color:'rgba(255,255,255,.35)', marginTop:1 }}>{p.nota}</div></div>
                  </div>)}
                  <div style={{ fontSize:9, fontWeight:800, color:'rgba(255,255,255,.4)', letterSpacing:'1px', marginBottom:6 }}>{tx.demo_gifts}</div>
                  {tx.demo_gift_items.map((g,i) => <div key={i} style={{ display:'flex', justifyContent:'space-between', padding:'6px 8px', background:'rgba(255,255,255,.04)', borderRadius:8, marginBottom:5 }}>
                    <span style={{ fontSize:12, color:g.taken?'rgba(255,255,255,.3)':'#fff', textDecoration:g.taken?'line-through':'none' }}>{g.name}</span>
                    <span style={{ fontSize:10, fontWeight:700, color:g.taken?'#a89df0':'#4ade80' }}>{g.taken?(lang==='en'?'Reserved':'Reservado'):(lang==='en'?'Available':'Disponible')}</span>
                  </div>)}
                  <button onClick={loginConGoogle} style={{ width:'100%', marginTop:8, padding:'11px', background:'linear-gradient(135deg,#534AB7,#D4537E)', border:'none', borderRadius:10, color:'#fff', fontSize:13, fontWeight:800, cursor:'pointer', fontFamily:F }}>{tx.demo_rsvp}</button>
                </div>
              </div>
      ...
      {/* CÓMO FUNCIONA */}
      <section id="como-funciona" style={{ padding:isMobile?'64px 20px':`100px ${px}`, position:'relative', zIndex:1 }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)', gap:16 }}>
            {tx.steps.map((s,i) => <div key={s.n} className="card-h" style={{ padding:isMobile?'28px 22px':'36px 28px', background:'rgba(255,255,255,.03)', border:'1px solid rgba(255,255,255,.07)', borderRadius:24, position:'relative', overflow:'hidden' }}>
              <div style={{ position:'absolute', top:-10, right:16, fontSize:isMobile?60:80, fontWeight:900, color:['rgba(83,74,183,.15)','rgba(212,83,126,.15)','rgba(168,157,240,.15)'][i], letterSpacing:'-4px', lineHeight:1 }}>{s.n}</div>
              <div style={{ width:40, height:40, borderRadius:12, background:['rgba(83,74,183,.2)','rgba(212,83,126,.2)','rgba(168,157,240,.2)'][i], border:`1px solid ${['rgba(83,74,183,.4)','rgba(212,83,126,.4)','rgba(168,157,240,.4)'][i]}`, display:'flex', alignItems:'center', justifyContent:'center', marginBottom:18 }}>
                <span style={{ fontSize:13, fontWeight:800, color:['#a89df0','#f08cb0','#c4bbf8'][i] }}>{s.n}</span>
              </div>
      ...
      {/* PRECIOS */}
      <section id="precios" style={{ padding:isMobile?'64px 20px':`100px ${px}`, position:'relative', zIndex:1 }}>
        <div style={{ maxWidth:1100, margin:'0 auto' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:isMobile?'1fr':'repeat(3,1fr)', gap:16 }}>
            {tx.plans.map(p => <div key={p.name} className="card-h" style={{ padding:isMobile?'24px 20px':'32px 24px', borderRadius:24, background:p.featured?'linear-gradient(160deg,#534AB7,#7b46a8 50%,#D4537E)':'rgba(255,255,255,.03)', border:p.featured?'none':'1px solid rgba(255,255,255,.07)', position:'relative', boxShadow:p.featured?'0 20px 60px rgba(212,83,126,.4)':'none' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:9, marginBottom:24 }}>
                {p.features.map(f => <div key={f} style={{ display:'flex', alignItems:'center', gap:9 }}>
              ...
              <button onClick={loginConGoogle} style={{ width:'100%', padding:'12px', background:p.featured?'#fff':'rgba(255,255,255,.07)', color:p.featured?'#534AB7':'rgba(255,255,255,.55)', border:p.featured?'none':'1px solid rgba(255,255,255,.1)', borderRadius:12, fontSize:14, fontWeight:800, cursor:'pointer', fontFamily:F }}>{p.cta}</button>
            </div>)}
          </div>
        </div>
      </section>
      ...
      {/* CTA FINAL */}
      <section style={{ padding:isMobile?'64px 20px':`100px ${px}`, position:'relative', zIndex:1, textAlign:'center' }}>
        <div style={{ maxWidth:560, margin:'0 auto', position:'relative' }}>
          ...
          <h2 style={{ fontSize:isMobile?34:52, fontWeight:900, letterSpacing:isMobile?'-1.5px':'-2px', color:'#fff', margin:'0 0 14px', lineHeight:1.05, position:'relative' }}>{tx.cta_title}</h2>
          <p style={{ fontSize:isMobile?15:17, color:'rgba(255,255,255,.4)', margin:'0 0 36px', position:'relative' }}>{tx.cta_sub}</p>
          <button className="btn-p" onClick={loginConGoogle} style={{ display:'inline-flex', alignItems:'center', gap:10, padding:isMobile?'14px 24px':'17px 32px', background:'linear-gradient(135deg,#534AB7,#D4537E)', border:'none', borderRadius:16, color:'#fff', fontSize:isMobile?15:17, fontWeight:800, cursor:'pointer', fontFamily:F, boxShadow:'0 16px 40px rgba(212,83,126,.45)', position:'relative', width:isMobile?'100%':'auto', justifyContent:'center' }}>
            <svg width="18" height="18" viewBox="0 0 48 48"><path fill="#FFC107" d="M43.611 20.083H42V20H24v8h11.303c-1.649 4.657-6.08 8-11.303 8-6.627 0-12-5.373-12-12s5.373-12 12-12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 12.955 4 4 12.955 4 24s8.955 20 20 20 20-8.955 20-20c0-1.341-.138-2.65-.389-3.917z"/><path fill="#FF3D00" d="m6.306 14.691 6.571 4.819C14.655 15.108 18.961 12 24 12c3.059 0 5.842 1.154 7.961 3.039l5.657-5.657C34.046 6.053 29.268 4 24 4 16.318 4 9.656 8.337 6.306 14.691z"/><path fill="#4CAF50" d="M24 44c5.166 0 9.86-1.977 13.409-5.192l-6.19-5.238A11.91 11.91 0 0 1 24 36c-5.202 0-9.619-3.317-11.283-7.946l-6.522 5.025C9.505 39.556 16.227 44 24 44z"/><path fill="#1976D2" d="M43.611 20.083H42V20H24v8h11.303a12.04 12.04 0 0 1-4.087 5.571l6.19 5.238C36.971 39.205 44 34 44 24c0-1.341-.138-2.65-.389-3.917z"/></svg>
            {tx.cta_btn}
          </button>
          <p style={{ fontSize:12, color:'rgba(255,255,255,.2)', marginTop:12, position:'relative' }}>{tx.no_cc2}</p>
        </div>
      </section>
