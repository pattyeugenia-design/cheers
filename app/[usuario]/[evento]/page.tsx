...
const EMOJIS: Record<string, string> = {
  cumple: '🎂', cumpleanos: '🎂', cena: '🍽️', viaje: '✈️',
  reunion: '🤝', evento: '🎉', otro: '✨'
}

const TINFO: Record<string, { icon: string; title: string }> = {
  invitados:   { icon: '👥', title: 'Invitados' },
  aperturas:   { icon: '👀', title: 'Quién abrió el link' },
  regalos:     { icon: '🎁', title: 'Lista de regalos' },
  itinerario:  { icon: '🗺️', title: 'Itinerario' },
  presupuesto: { icon: '💸', title: 'Presupuesto compartido' },
  quellevar:   { icon: '🎒', title: 'Qué llevar' },
  menu:        { icon: '🍽️', title: 'Quién trae qué' },
  reservacion: { icon: '📅', title: 'Reservación' },
  mensajes:    { icon: '💌', title: 'Mensajes para el festejado' },
}
...
export default function Dashboard({ params }: { params: Promise<{ usuario: string; evento: string }> }) {
  ...
  useEffect(() => {
    params.then(async ({ usuario, evento }) => {
      ...
      // buscar celebracion por slug usuario/evento o solo evento
      let cel: any = null
      ...
      // solo el organizador puede ver el dashboard
  ...
  if (acceso === 'denied') return (
    <div style={{ minHeight: '100vh', background: BG.morado.css, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: FONT }}>
      <div style={{ background: '#fff', borderRadius: 24, padding: '2.5rem', maxWidth: 400, textAlign: 'center', boxShadow: '0 24px 60px rgba(0,0,0,.25)' }}>
        ...
        <h1 style={{ fontSize: 22, fontWeight: 800, color: '#1c1830', margin: '12px 0 8px' }}>Acceso denegado</h1>
  ...
  const emoji = EMOJIS[celebracion?.tipo] || '✨'
    ...
    <div style={{ position: 'fixed', inset: 0, overflowY: 'auto', background: bg.css, padding: '26px 18px 44px', boxSizing: 'border-box', fontFamily: FONT }}>
      ...
      <div style={{ position: 'relative', maxWidth: 940, margin: '0 auto' }}>
        ...
        <div style={{ background: 'rgba(255,255,255,.97)', borderRadius: 26, overflow: 'hidden', boxShadow: '0 18px 46px rgba(25,12,50,.22)', marginBottom: 16 }}>
          ...
          <div style={{ padding: '16px 18px 20px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 16px', padding: '0 4px' }}>
              ...
              <div>
                <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '.4px', color: '#a39ec0', textTransform: 'uppercase', margin: '0 0 1px 8px' }}>Link público</div>
        ...
        <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 22, padding: '22px 24px', marginBottom: 18, background: 'linear-gradient(120deg,#534AB7,#7b46a8 55%,#D4537E)', boxShadow: '0 16px 42px rgba(83,74,183,.45)', color: '#fff' }}>
          <div style={{ position: 'absolute', top: -30, right: -10, fontSize: 120, opacity: 0.16, lineHeight: 1 }}>🥂</div>
        ...
        {/* Hint */}
        <div style={{ fontSize: 13, fontWeight: 600, color: textColor, opacity: 0.8, margin: '0 4px 12px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          {tiles.map((tile, i) => {
            const info = TINFO[tile.key] || { icon: '✦', title: tile.key }
            ...
            return (
              <div
                ...
                style={{ gridColumn: isLg ? '1 / -1' : 'auto', background: '#fff', borderRadius: 22, padding: '20px 18px', boxShadow: '0 8px 24px rgba(25,12,50,.1)', cursor: 'default' }}
              >
                      ...
                      {rsvps.slice(0, 6).map((r, j) => (
                        <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          ...
                          <span style={{ fontSize: 11, fontWeight: 700, color: r.asistencia === 'si' ? '#1f8a5b' : '#c98a1e', background: r.asistencia === 'si' ? '#ECF7F0' : '#FFF4E6', padding: '3px 10px', borderRadius: 99 }}>
                            {r.asistencia === 'si' ? '✅ Va' : r.asistencia === 'no' ? '❌ No' : '🤔 Tal vez'}
                          </span>
                        </div>
                      ))}
                    </div>
                ...
                {/* Aperturas — placeholder por ahora */}
                {tile.key === 'aperturas' && (
                  <div>
